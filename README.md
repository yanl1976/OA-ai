...
### 2026-09-12 · 会议纪要二次生成增强（议题级选择 + 省略说明行）

二次生成会议纪要 PDF 支持**按议题挑选**与**省略说明**：

- **议题级选择**：前端 `MeetingDerived.vue` 把正文按模板拆成议题列表，可勾选要保留的议题（全选/取消全选），选中议题自动从「一、」顺延重编号；未选议题不进 PDF。
- **省略说明行**：存在未选议题时，在「议题」与「出席人员」之间自动插入加粗说明行（默认「其他事项略去」，文案可改）。`pdf_make.py` 以黑体、左对齐、首行缩进 2 字渲染。
- 后端 `derived_store.py` 同步解析/生成；正文未识别为模板时回退段落块模式。

涉及文件：`web_vue/src/components/MeetingDerived.vue`、`app/pdf_make.py`、`app/derived_store.py`。
...
- **议题级选择**：纪要类文档点「纪要二次生成」，前端按模板把正文拆成议题列表，可勾选需保留的议题（全选/取消全选），选中议题自动从「一、」顺延重编号；未选议题不进 PDF。
- **省略说明行**：存在未选议题时，在「议题」与「出席人员」之间自动插入加粗说明行（默认「其他事项略去」，文案可改）；`pdf_make.py` 以黑体、左对齐、首行缩进 2 字渲染。
- 正文未识别为模板时回退段落块模式。后端 `derived_store.py` 同步。
