...
_PARAGRAPH_START = re.compile(
    ...
    r"|(?:出席|列席|参加|参会|缺席|请假)人员\s*[:：]"        # 出席/列席人员：（行内要素，从行首匹配）
    r"|(出席|列席|主持|审阅|记录|抄送|印发|主题词|报送)\s*[:：]"
    r")"
)
# 句末标点（当前行以此结尾视为段落结束）
_END_PUNCT = set("。！？；：”’）】」…：…")
# 行内议题起始（排版公理：议题序号标记新段落起点）：在「句末标点 + 议题序号」处
# 断行，把 PDF 抽取粘连的相邻议题（如「…事项。三、关于…议题」）恢复到各自独立段落。
# 仅当序号前是句末标点 / 分号时才断——与公文排版事实一致（议题间必有句号分隔），
# 避免误切正文里的普通列举；序号后的 (?![\d]) 排除「923.51」「1.5亿」等金额/小数。
_INLINE_ITEM_START_RE = re.compile(
    r"(?<=[。！？!?；])\s*([一二三四五六七八九十百零\d]+[、.．](?![\d]))"
)
# 行内公文要素前缀（排版公理：公文要素标记新段落起点）：在行内任意位置出现时断行，
# 把 PDF 抽取粘连的相邻要素（如「…崔纪斌列席人员：张峰…」）恢复到各自独立段落。
# 要素前缀本身是强排版边界，不会出现在正文句中，故无需限定前导标点。。
_INLINE_ELEM_START_RE = re.compile(
    r"((?:出席|列席|参加|参会|缺席|请假)人员\s*[:：]"
    r"|(?:主持|审阅|记录|抄送|印发|报送|主题词)\s*[:：])"
)

# 议题序号头行（仅含序号、未含句末标点）：用于在续行合并时识别「议题标题被 PDF
# 文本块边界折成两行」的场景。如「一、关于发放2025年」+「暖气补贴的议案」本属同一
# 标题行，但因序号行命中 _PARAGRAPH_START，默认续行逻辑会误把它当成新段落起点而把
# 标题斩断。该正则与 _PARAGRAPH_START 的议题序号分支同源，但要求整行未以句末标点收尾
# （即标题尚未结束），用于守卫续行拼回。标题可含数字（如年份「2025」），故用
# 「非句末标点字符」而非「仅中文」界定剩余部分。
_ITEM_HEAD_ONLY_RE = re.compile(
    ...
    for raw_ln in expanded:
        ...
        if s == "":
            ...
            if cur:
                ...
                cur = ""
            continue
        # 行内议题起始断行（排版公理：议题序号标记新段落起点）：
        # PDF 抽取常把相邻议题粘连在同一行（如「…决定上述事项。三、关于李刚刚…议题」），
        # 在「句末标点 + 议题序号」处插入换行，恢复各议题为独立段落。
        # 只匹配「句末标点 + 议题序号」，行首议题（其前无句末标点）不会被误切，
        # 因此无需「行首跳过」守卫——行内后续的其他议题序号（即便本行以议题开头）同样断开。
        s = _INLINE_ITEM_START_RE.sub(r"\n\1", s)
        # 行内公文要素前缀断行（排版公理：公文要素标记新段落起点）：
        # 把粘连的相邻要素（如「…崔纪斌列席人员：张峰…」）恢复到各自独立段落。
        s = _INLINE_ELEM_START_RE.sub(r"\n\1", s)
        # 对分裂出的多行（或原单行）逐个纳入段落流
        for sub in s.split("\n"):
            sub = sub.strip()
            if not sub:
                continue
            if cur == "":
                cur = sub
            elif _ATTEND_LINE_RE.match(cur) and not cur.rstrip().endswith("。") and not _ATTEND_LINE_RE.match(sub) and not _PARAGRAPH_START.match(sub):
                # 名单续行（同一要素）：出席/列席等人员名单被 PDF 块边界切断
                # （如「列席人员：张」与「峰、赵建宏…」分两行），此时 cur 是未结束的
                # 名单行，应优先合并续行而非被 _PARAGRAPH_START 误判为公文要素新段落而断开。
                # 但若 sub 本身是另一要素前缀（如「列席人员：」），则为新要素段落，断段。
                cur = cur + sub
            elif _ITEM_HEAD_ONLY_RE.match(cur) and not _ITEM_HEAD_ONLY_RE.match(sub) and not _PARAGRAPH_START.match(sub) and not _END_PUNCT.intersection(cur[-1:]):
                # 议题标题续行守卫：cur 是未结束的议题序号头行（如「一、关于发放2025年」），
                # 而 sub 不是新议题/公文要素（不命中 _PARAGRAPH_START、不是另一议题序号头、
                # cur 也未以句末标点收尾——标题尚未结束），则 sub 是标题被 PDF 文本块边界
                # 折断的续行（如「暖气补贴的议案」），应拼回 cur 构成完整标题。
                # 拼回后在「议案/议题/事项」等标题终止词后强制断段：标题独立成行，
                # 其后的正文（如「会议审议了该项议案。」）作为新段落起始，绝不被糊进标题。
                merged = cur + sub
                m = re.search(r"(?:议案|议题|事项)[：:。．]?", merged)
                if m and m.end() < len(merged):
                    # 标题终止词之后还有正文 -> 标题独立成段，正文另起
                    title = merged[:m.end()].rstrip("：:。．")
                    paras.append(title)
                    cur = merged[m.end():].lstrip("：:。． ")
                else:
...
def detect_kind(text):
    ...
    # 【放宽要素词表·修复纪要退化】原仅认 5 个词（会议决定/出席人员/列席人员/主持人/
    # 会议时间），实际纪要常用「参会人员」「参加会议」「列席会议」「会议由…主持」
    # 「现将…纪要如下」「会议审议」等表述，一旦全部落空即退化为 default，
    # 走 process_default 会把红头 5 元素粘连成一行（表现为「提取乱套」）。
    # 此处扩充为常见公文表述，并要求同时存在纪要题名特征，避免误判标准类。
    _MIN_ELEMS = (
        r"会议决定|出席人员|列席人员|主持人|会议时间|参会人员|参加人员|"
        r"参加会议|列席会议|会议由|会议审议|会议听取|纪要如下|现纪要如下|"
        r"现将会议|参会|出席|列席|记录人|议题"
    )
    if "会议纪要" in head and re.search(_MIN_ELEMS, body):
        return "minutes"
    # 次优先：正文含典型纪要导语（无『会议纪要』题名时，如扫描件题名缺失）
    if re.search(r"现将会议有关事项纪要如下|现将会议.*纪要如下", body):
        return "minutes"
